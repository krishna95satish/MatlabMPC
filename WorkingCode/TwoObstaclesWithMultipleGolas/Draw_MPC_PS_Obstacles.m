function Draw_MPC_PS_Obstacles (state_iter, t,xx,xx1,u_cl,xs,N,rob_diam,obs_history,obs_x,obs_y, obs_x1,obs_y1,obs_diam)
...
for k = 1:size(xx,2)
    ...
    plot(x1+xp,y1+yp,'--r'); % plot robot circle
     ...
     [~, len] = size(obs_history); 
     if k >= state_iter 
        if (counter_obs_pos <= len)
            plot(obs_history(:,counter_obs_pos)+xp_obs,obs_y1+yp_obs,'--b'); % plot obstacles circle
            counter_obs_pos = counter_obs_pos + 1;
        else
            plot(obs_history(:,len)+xp_obs,obs_y1+yp_obs,'--b'); % plot obstacles circle
        end
     else
        plot(obs_x+xp_obs,obs_y+yp_obs,'--b'); % plot obstacles circle  
        plot(obs_x1+xp_obs,obs_y1+yp_obs,'--b'); % plot obstacles circle
     end

...
rhs = [v*cos(theta);v*sin(theta);a; (v*(tan(delta)/WB)) + (v*delta*tan(delta)^2) + delta/WB];
...
args.lbx(4:4:4*(N+1),1) = -inf; 
args.ubx(4:4:4*(N+1),1) = inf; 
...
for i = 1:s
   ...
   %% args.ubx = 10; test for dynamically changind the road boundaries ! It seems to work
    ...
    [t0, x0, u0] = Test_Shift(T, t0, x0, u,f);
